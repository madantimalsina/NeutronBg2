...
void CountMSSS::Initialize() {
 ...
 //SS
  m_hists->BookScatterPlot("SS_All");
  m_hists->BookScatterPlot("SS_All_FV");
 ...
 // MS
  m_hists->BookScatterPlot("MS_All");
  m_hists->BookScatterPlot("MS_All_FV");
...
void CountMSSS::Execute() {
  ...
  if (ETrain == false ){
  if (IsNeutron_PTFE == true || (*m_event->m_TruthEvent)->parentParticle == "neutron" || (*m_event->m_TruthEvent)->parentParticle == "USF_neutron")
    { m_hists->BookFillHist("PEn_AllNeutrons", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); }
  //SS
  //if (IsNeutron_PTFE == true || IsNeutron_neutron == true || IsNeutron_USF == true && (numSS > 0 && ETrain == false))
  ...
  if(m_cuts->mdc3()->WIMPSearchROI())
    { m_hists->BookFillHist("PEn_SS_ROI", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); }
  
  // Skin
  if (!skinVeto)
  { m_hists->BookFillHist("PEn_SS_Skin", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); }
  // OD
 if (!odVeto)
  { m_hists->BookFillHist("PEn_SS_OD", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); }
  // Skin and OD
  if (!skinVeto && !odVeto)
  { m_hists->BookFillHist("PEn_SS_Skin_OD", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); }
  //FV
...
*/
  ...
  if (IsNeutron_PTFE == true || (*m_event->m_TruthEvent)->parentParticle == "neutron" || (*m_event->m_TruthEvent)->parentParticle == "USF_neutron"){
      ...
      m_hists->BookFillHist("PEn_SS_2", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); 

      std::cout << "SS(all): rawFile:  "<< rawFileName<< ",  " << runID << ",  " << eventID<< ",  " << S1c_ss << ",  " << S2c_ss << endl << endl;
        if ((*m_event->m_TruthVertices)->nRQMCTruthVertices > 0){
          for (int v = 0; v < (*m_event->m_TruthVertices)->nRQMCTruthVertices; v++){
            if ((*m_event->m_TruthVertices)->particleName[v] == "proton"){
        std::cout << "SS(PP): rawFile:  "<< rawFileName<< ",  " << runID << ",  " << eventID<< ",  " << S1c_ss << ",  " << S2c_ss << endl << endl;}}}

      m_hists->BookFillHist("SS_LogS1S2_All",5000,s1_min,10000, nBinsS1, 2, 7, S1c_ss, S2c_ss);
      m_hists->GetScatterPlot("SS_All")->Fill( S1c_ss, S2c_ss);
    ...
    //odVeto = m_cuts->mdc3()->ODVeto();
            if (((*m_event->m_TruthVertices)->nRQMCTruthVertices > 0)) {
              ...
              m_hists->BookFillHist("SS_ZRTruthVertexPosition_All", 100, 0., 1500., 100, -1000., 3000., rPos, zPos);}
  
  //ROI
  if(m_cuts->mdc3()->WIMPSearchROI())
    { m_hists->BookFillHist("PEn_SS_ROI_2", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); }
  
  // Skin
  if (!skinVeto)
  { m_hists->BookFillHist("PEn_SS_Skin_2", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); }
  // OD
 if (!odVeto)
  { m_hists->BookFillHist("PEn_SS_OD_2", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); }
  // Skin and OD
  if (!skinVeto && !odVeto)
  { m_hists->BookFillHist("PEn_SS_Skin_OD_2", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); }
  //FV
  if (m_cuts->mdc3()->Fiducial())
    { m_hists->BookFillHist("PEn_FV_2", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV);
      m_hists->GetScatterPlot("SS_All_FV")->Fill( S1c_ss, S2c_ss); 

        if ((*m_event->m_TruthVertices)->nRQMCTruthVertices > 0){
          for (int v = 0; v < (*m_event->m_TruthVertices)->nRQMCTruthVertices; v++){
            if ((*m_event->m_TruthVertices)->particleName[v] == "proton"){
        std::cout << "SS(FV_): rawFile:  "<< rawFileName<< ",  " << runID << ",  " << eventID<< ",  " << S1c_ss << ",  " << S2c_ss << endl << endl;}}}
    }
...
// For MS
  ...
  if (IsNeutron_PTFE == true || (*m_event->m_TruthEvent)->parentParticle == "neutron" || (*m_event->m_TruthEvent)->parentParticle == "USF_neutron"){
      ...
      m_hists->BookFillHist("PEn_MS_2", 100, 0., 10000., (*m_event->m_TruthEvent)->parentEnergy_keV); 

      std::cout << "MS(all): rawFile:  "<< rawFileName<< ",  " << runID << ",  " << eventID<< ",  " << S1c_ms << ",  " << logS2c_ms << endl << endl;
    if ((*m_event->m_TruthVertices)->nRQMCTruthVertices > 0){
      for (int v = 0; v < (*m_event->m_TruthVertices)->nRQMCTruthVertices; v++){
        if ((*m_event->m_TruthVertices)->particleName[v] == "proton"){
      std::cout << "MS(PP): rawFile:  "<< rawFileName<< ",  " << runID << ",  " << eventID<< ",  " << S1c_ms << ",  " << logS2c_ms << endl << endl;}}}

      m_hists->BookFillHist("MS_LogS1S2_All",55000,s1_min,110000, nBinsS1, 2, 7, S1c_ms, logS2c_ms);
      m_hists->GetScatterPlot("MS_All")->Fill( S1c_ms, logS2c_ms);
            ...
            if (((*m_event->m_TruthVertices)->nRQMCTruthVertices > 0)) {
              ...
              m_hists->BookFillHist("MS_ZRTruthVertexPosition_All", 100, 0., 1500., 100, -1000., 3000., rPos, zPos);}    
    ...
    if (numMS > 0 && S1c_ms < 500. && Rc_ms < 68.8 && Zc_ms > 1.5 && Zc_ms < 132.1 && ETrain == false){
          ...
          m_hists->GetScatterPlot("MS_All_FV")->Fill( S1c_ms, logS2c_ms);
        
            if ((*m_event->m_TruthVertices)->nRQMCTruthVertices > 0){
      for (int v = 0; v < (*m_event->m_TruthVertices)->nRQMCTruthVertices; v++){
        if ((*m_event->m_TruthVertices)->particleName[v] == "proton"){
      std::cout << "MS(FV_): rawFile:  "<< rawFileName<< ",  " << runID << ",  " << eventID<< ",  " << S1c_ms << ",  " << logS2c_ms << endl << endl;}}}
...
void CountMSSS::Finalize() {
 ...
 //SS
  m_hists->DrawERNRBands("SS_LogS1S2_All", "log(s2)");
  ...
  m_hists->DrawERNRBandsScatterPlot("SS_All", "log(s2)");
  m_hists->DrawERNRBandsScatterPlot("SS_All_FV", "log(s2)");
  m_hists->DrawERNRBandsScatterPlot("SS_All_FV", "log(s2)");
  ...
  // MS
  m_hists->DrawERNRBands("MS_LogS1S2_All", "log(s2)");
  ...
  m_hists->DrawERNRBandsScatterPlot("MS_All", "log(s2)");
  m_hists->DrawERNRBandsScatterPlot("MS_All_FV", "log(s2)");
  m_hists->DrawERNRBandsScatterPlot("MS_All_FV", "log(s2)");
